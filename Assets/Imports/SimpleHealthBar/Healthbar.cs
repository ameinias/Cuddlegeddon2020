...
public class Healthbar : MonoBehaviour {
   ...
   public float totalHealth;
    ...
    private void Update()
    {
        ...
        if (minimumHealth < 0)
        {
            tempMin = minimumHealth * -1;
            totalHealth = maximumHealth + tempMin; }
        else { totalHealth = maximumHealth - minimumHealth;  }

        healthPercentage = int.Parse((Mathf.Round(totalHealth * (health / 100f))).ToString());
